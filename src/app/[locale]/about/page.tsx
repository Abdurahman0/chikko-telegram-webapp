...
function AboutScreen({ locale }: { locale: AppLocale }) {
  ...
  return (
    <div className="space-y-4">
      <Link
        href={`/${locale}/profile`}
        className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-surface text-brand-strong shadow-soft ring-1 ring-surface-accent"
        aria-label="Back"
      >
        ←
      </Link>
      ...
      <div className="rounded-3xl bg-surface p-5 text-center shadow-soft">
        <div className="mx-auto mb-3 inline-flex h-14 w-14 items-center justify-center rounded-full bg-brand-soft text-xl">
          👶
        </div>
      ...
      <div className="rounded-3xl bg-surface p-4 shadow-soft">
        ...
        <div className="mt-3 space-y-2">
          {workSchedule.map((day) => {
            ...
            const hours =
              day.hours === "closed" ? messages.about.closed : day.hours;
            return (
              <div
                key={day.key}
                className={cn(
      ...
      <div className="rounded-3xl bg-surface p-4 shadow-soft">
        ...
        <div className="mt-3 flex items-center gap-2">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 rounded-full bg-surface-soft px-3 py-2 text-sm"
          >
            📷 {messages.about.instagram}
          </a>
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full bg-surface-soft px-3 py-2 text-sm"
          >
            ✈️ {messages.about.telegram}
          </a>
        </div>
        <a
          href="https://chikko.uz"
          target="_blank"
          rel="noreferrer"
          className="mt-3 inline-block text-sm font-semibold text-brand-strong"
        >
          {messages.about.website}: chikko.uz
        </a>
      </div>
    </div>
  );
